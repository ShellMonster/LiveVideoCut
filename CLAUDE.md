...
- **assets/:taskId/:segmentId/commerce** — 独立 AI 商品素材工作台，调用 Gemini 商品识别/平台文案生成，并调用 OpenAI Image/gpt-image-2 生成模特图和淘宝详情页示例
...
AI 商品素材后端接口：`GET /api/commerce/clips/{task_id}/{segment_id}` 返回工作台数据；`POST /api/commerce/clips/{task_id}/{segment_id}/analyze` 调 Gemini `generateContent` 识别封面并写 `product_analysis.json`；`POST /api/commerce/clips/{task_id}/{segment_id}/copywriting` 调 Gemini 生成抖音/淘宝文案并写 `copywriting.json`；`POST /api/commerce/clips/{task_id}/{segment_id}/images` 调 OpenAI Image `/images/edits`，以片段封面作为参考图生成正面、侧面、背面/细节和淘宝详情页示例图，写入 `commerce/<segment_id>/images/` 与 `images.json`。
