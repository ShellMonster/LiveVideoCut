...
这些设置优先保存在后端 `uploads/app_config.sqlite3`，`.env` 作为首次启动/缺省兜底，前端 Zustand store + localStorage 只作为页面状态和本地副本；只影响之后新上传的任务。
