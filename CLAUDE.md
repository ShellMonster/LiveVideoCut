...
- **assets/:taskId/:segmentId/commerce** — 独立 AI 商品素材工作台，异步调用 Gemini 商品识别/平台文案生成，并异步调用 OpenAI Image/gpt-image-2 生成模特图和淘宝详情页示例；支持单张素材图独立重试
...
- 前端：`AssetsPage.tsx` 采用项目分组卡片 + 右侧片段详情抽屉 + 底部批量选择条；批量 AI 素材提交成功后打开右侧队列抽屉展示已排队和失败项；批量 ZIP 下载单次最多 20 个片段，超过时前端禁用下载并提示限制。
