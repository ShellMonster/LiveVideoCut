...
- **diagnostics** — 任务诊断（项目工具栏、指标卡、横向管线阶段、漏斗图、异常建议、分页事件日志、右侧事件详情、导出报告/artifacts.zip）
...
- 前端：`DiagnosticsPage.tsx` 采用设计图式诊断仪表盘：顶部项目工具栏、紧凑指标卡、横向流水线耗时、片段漏斗、右侧异常建议/事件详情和分页事件日志；窄屏下指标与流水线自动降密度展示，事件详情展示完整消息、阶段、级别、文件来源和处理建议。
