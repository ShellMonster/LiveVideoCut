...
- **settings** — 设置编辑器（侧栏+面板布局；顶层页签拆为处理预设、AI 服务、转写服务、字幕设置、敏感词过滤、切分策略、导出与音频、高级参数，避免不同性质的设置挤在一个大表单里；字幕位置编辑器使用左侧设置项 + 右侧 9:16 视频模拟预览的左右布局；页面展示枚举值时必须转中文，不直接显示 `subtitle_position`、`middle`、`no_vlm` 等内部 key）
