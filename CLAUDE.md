...
- 字幕参数：
  ...
  - `subtitle_font_size`: 普通字幕字号，默认 `45`，basic/styled 通过 FFmpeg `force_style` 生效，karaoke 用作 Default 样式字号
  - `subtitle_highlight_font_size`: karaoke 逐字高亮层字号，默认 `55`
  - `subtitle_template`
...
这些设置的用户覆盖项保存在后端 `uploads/app_config.sqlite3`，未覆盖字段继续使用 `.env` 或代码默认值；前端 Zustand store + localStorage 只作为页面状态和本地副本；只影响之后新上传的任务。
...
- `uploads/<task_id>/secrets.json`（敏感字段快照，权限 600）
...
- `uploads/app_config.sqlite3`（全局设置真源，优先于 `.env`）
...
- `README.md` 顶部产品流程图下方展示 `docs/images/screenshots/settings-ai.png`，用于说明 AI 服务设置界面
