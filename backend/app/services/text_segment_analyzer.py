...
SYSTEM_PROMPT = """你是一位直播视频内容分析专家。你的任务是分析直播转写文本，识别主播讲解的商品/服装切换边界。
  ...
  "segments": [
    {
      ...
      "boundary_reason": "判断边界的原因，如：主播说'接下来看这款'、开始描述新商品特征等"
    }
  ]
}
...
3. 每个segment应该对应一个相对完整的商品讲解段落
4. confidence 反映你对边界判断的把握程度
5. 如果无法确定商品切换，输出空 segments 数组
...
class TextSegmentAnalyzer:
    ...
    def analyze(self, transcript: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        logger.info(
            "Analyzing transcript: %d segments, %.1fs total",
            len(transcript),
            total_duration,
        )
