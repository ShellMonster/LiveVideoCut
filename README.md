...
1. **配置设置** — 在设置页配置 VLM / ASR / 字幕 / 切分等参数。后端 `uploads/app_config.sqlite3` 只保存用户覆盖项，未覆盖字段继续跟随 `.env` 或代码默认值，浏览器 localStorage 仅作本地副本；保存后的配置只影响之后新上传的任务，已有任务不受影响。页面按业务分 8 个页签（处理预设、AI 服务、转写服务、字幕设置、敏感词过滤、切分策略、导出与音频、高级参数），枚举值全部中文展示。
...
字幕位置支持四种预设（top / middle / bottom）和自定义模式（拖拽调整纵向百分比）。普通字幕和高亮字幕字号可分别设置，默认普通层 45，高亮层 55；设置页右侧 9:16 演示预览中的字幕条采用较宽示意条，便于观察位置。
...
全局设置读取顺序为用户在 `uploads/app_config.sqlite3` 中保存的覆盖项 → `.env` 环境变量 → 代码默认值。设置页保存时只写入相对当前有效配置发生变化的字段；清空某个字段或恢复默认会删除对应覆盖项，使其重新跟随 `.env` 或代码默认值；未保存状态也基于同一套字段 diff 判断，避免对象顺序导致误报。API Key 输入框默认闭眼隐藏，点击眼睛按钮可在前端查看和编辑明文。新任务上传时会把当时生效的配置固化到任务目录，非敏感配置写入 `settings.json`，敏感字段写入 `secrets.json`。
...
- **病毒传播潜力评分** — 每个片段自动生成 0-100 传播潜力分，复用已有信号 + LLM 评分
- **Hook / 黄金 3 秒检测** — 自动找到片段内最吸引人的开头，重排起止点
- **AI 标题 + Hashtag 自动生成** — 每个片段自动生成抖音/小红书风格的标题、文案和话题标签
- **自动精彩集锦** — 一键从所有片段中选出最佳 5-8 个，拼成 30-90 秒精华混剪
- **画质增强** — 可选的 Real-ESRGAN 超分辨率 + GFPGAN 人脸修复（需 GPU）

### Phase 2：内容分发（3-6 个月）

- **多平台视频适配** — 一次导出自动生成抖音（9:16）、小红书（3:4）、视频号、淘宝等多版本
- **小红书图文 / 产品海报** — 从片段自动生成图文帖（封面图 + 卖点文案 + Hashtag）和商品海报
- **Transcript → 种草文 / 博客** — 从转写文本自动生成小红书种草文、公众号长文、SEO 博客
- **商品知识库（RAG）** — 从多场直播积累商品知识，支持语义搜索和智能问答

### Phase 3：管线升级（6-12 个月）

- **双人直播支持** — ByteTrack 人物追踪 + 分人换衣检测 + 人脸锚定 ID + 说话人分离
- **开源自部署 ASR** — FunASR / SenseVoice 替代云 ASR，降低长期成本
- **语音克隆 / AI 旁白** — CosyVoice / Fish Speech 克隆主播声音，自动生成商品旁白或多语言版本
- **商品链接 / 可购视频** — 片段自动关联商品库，生成可交互播放器和片尾商品卡片
- **Gemini 3 长视频分析** — 用 Gemini 3 Flash 一次分析 3 小时视频（$0.69/次），替代逐帧 VLM 调用

### Phase 4：探索方向（12 个月+）

- **实时直播剪辑** — 边播边切，直播结束即出片
- **数字人商品视频** — 商品文案自动生成数字人口播讲解视频
- **虚拟试穿** — 商品图 → AI 模特试穿图，集成到 Commerce Workbench
- **批量排期发布** — 多平台自动排期，按最佳时间发布

📄 [完整路线图文档](docs/roadmap.md) | [双人直播方案](docs/dual-person-live-stream-proposal.md)
...
本项目允许个人和商业场景在 AGPL-3.0 条款下使用、修改和分发；如果你通过网络提供基于本项目修改后的服务，也需要按 AGPL-3.0 向用户提供对应源码。
