...
- **任务队列** -- 队列流式任务列表 + 可关闭右侧任务详情抽屉 / 窄屏任务卡片，聚合阶段进度、最近日志、Worker 资源、重试和删除；当前阶段统一中文展示，操作列使用固定图标按钮避免挤压
- **剪辑复核** -- 卡片式片段队列 + 右侧复核抽屉，支持状态筛选、片段预览、字幕只读预览、AI 建议和单片段重导出
- **片段资产** -- 按项目分组的资产卡片 + 右侧详情抽屉 + 底部批量选择条，支持服务端筛选、分页、预览和下载
- **音乐库** -- 曲目列表 + 右侧标签编辑抽屉 + 底部播放器，支持上传、播放、标签编辑和用户曲目删除
- **任务诊断** -- 流水线耗时、片段漏斗、异常建议和事件日志，点击日志可打开右侧事件详情抽屉
...
- 剪辑复核采用片段卡片网格和右侧抽屉，抽屉包含详情、字幕、AI 建议三个页签；字幕页签当前为只读预览，避免前端文本与已烧录视频字幕不一致。
- 片段资产采用按项目分组的卡片集合，点击片段打开右侧详情抽屉；多选后的总时长、预计 ZIP 和批量下载入口固定在底部浮动条，单次批量 ZIP 仍限制最多 20 个片段。
- 音乐库采用曲目列表和右侧标签编辑抽屉，底部播放器显示当前曲目、播放进度和默认音量。
- 诊断报告保留分页事件日志，事件行点击后打开右侧详情抽屉展示完整消息、阶段、级别和文件来源。
